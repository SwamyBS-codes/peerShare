        ...
        {files.length === 0 ? (
          ...
          <div className="max-h-60 overflow-y-auto pr-1 space-y-2">
            {files.map((file, idx) => (
              <div
                key={`${file.name}-${idx}`}
                className="flex items-center justify-between rounded-2xl border border-slate-200/60 bg-white/70 p-3 text-sm shadow-sm dark:border-slate-800/80 dark:bg-slate-900/60 transition hover:border-slate-300 dark:hover:border-slate-700 min-w-0"
              >
                <div className="flex items-center gap-3 max-w-[80%] min-w-0">
                  <div className="p-2 rounded-lg bg-indigo-500/10 text-indigo-500 dark:bg-indigo-400/15 dark:text-indigo-400 flex-shrink-0">
